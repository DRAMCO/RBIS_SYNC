...
void IRAM_ATTR callbacktimer1(){
  ...
  togglesettimer = !togglesettimer; //toggle gpio pin
  digitalWrite(settimer, togglesettimer);
  if(!connectedtowifi){
   // esp_wifi_80211_tx(WIFI_IF_STA, beaconPacket, sizeof(beaconPacket) , false); // send beacon frame to slave, uncomment for slave extension
  }
  else{
...
void IRAM_ATTR sniffercallback(void* buf, wifi_promiscuous_pkt_type_t type) { //This is where packets end up after they get sniffed
  ...
  */
  ...
  if(framecontrol->subtype==8){
    ...
    // do stuff if it is the right access point
    uint64_t id;
    ...
    case SENDREFERENCES:{
      ...
      while(true){// send data to slave
        ...
        if(ctr>4){ // stop trying to connect after 4 times
            Serial.println("no refs sent");
            break;
        }
      }
      appState = IDLE;
      delay(5000); //small delay so TM2 has time to send timestamps to TM3 
