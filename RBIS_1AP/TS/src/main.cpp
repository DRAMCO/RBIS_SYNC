...
void IRAM_ATTR callbacktimer1(){
  if(!connectedtowifi){//send beacon
    //esp_wifi_80211_tx(WIFI_IF_STA, beaconPacket, sizeof(beaconPacket) , false); //  send beacon frame to slaves; unncomment if slave extension is used
  }
  else{
    
  }
  togglesettimer = !togglesettimer;
  digitalWrite(settimer, togglesettimer);
...
void IRAM_ATTR sniffercallback(void* buf, wifi_promiscuous_pkt_type_t type) { //This is where packets end up after they get sniffed
  ...
  uint64_t time = timerRead(timer1);   //take timestamp
  /*
  ...
  */
  ...
  if(framecontrol->subtype==8){
    ...
    if(juisteMAC){// do stuf if it is the right access point
      ...
      if(!maxref){// check if the max amount of references is reached
...
void loop() {
  ...
  switch (appState) {
    ...
    case SETNEXTTIMER:{// set the next timer interrupt
      ...
      Sprint("current time= ");
      Sprintln(PriUint64<DEC>(timerRead(timer1)));
    }break;
    ...
    case RECEIVE:{
      ...
      // receive references from master
      while(true){
    ...
    if( n > 10){// do stuf if we have 10 or more matching timestamps
      ...
      int median = findMedian(offsets,n); // calculate median of offsets

      uint64_t time = timerRead(timer1)+median;
      timerWrite(timer1, timerRead(timer1)+median); // adjust timer based on median offset
