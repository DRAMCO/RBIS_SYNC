#include "_util.h"
// for wifi connection:
const char* ssid = "ssid-of-network"; // change to desired ssid
const char* pwd = "*********"; // change to correct pwd


/*
 * This is the IP address of the sever
*/
const char* host = "192.168.0.136"; // change to ip address of TS
const int port = 8088;
...
bool sendreferences(referencepoint refs[], int length){
  ...
  WiFi.begin(ssid,pwd); // no arguments if enterprise wifi is used
  int counter = 0;
  ...
  while (WiFi.status() != WL_CONNECTED) {
      ...
      Serial.print(".");
      counter++;
      if(counter>=10){ //after 5 seconds timeout - reset stop trying
